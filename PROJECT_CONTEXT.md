...
> Описание исходного MVP, не текущий план работ. С 14 сентября 2026 года
> приоритет — наполненные страницы → простая админка → законченный тест.
> Актуальные требования и порядок: [PROJECT_PLAN.md](PROJECT_PLAN.md),
> статусы: [docs/TASKS.md](docs/TASKS.md), правила: [AGENTS.md](AGENTS.md).
> Нижеследующий API описывает исходный контракт; новые функции не считаются готовыми.
...
Секрет подписи JWT: переменная окружения `JWT_SECRET`. Пароли в БД хранятся в виде хэша (passlib: argon2 / pbkdf2 / bcrypt).
